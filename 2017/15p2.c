...
{
  uint64_t p_a = 699, p_b = 124;
  uint64_t fac_a = 16807, fac_b = 48271;
  uint64_t d = 2147483647, i;
  uint64_t mask = 0x000000000000ffff;
  int count = 0, flip_flop = 1;

  for (i = 0; i < 5000000; ++i)
  {
    while((p_a = (p_a * fac_a) % d) % 4 != 0);
    while((p_b = (p_b * fac_b) % d) % 8 != 0);

    if ((p_a & mask) == (p_b & mask))
      count++;
  }

  printf("The judge's final count: %d\n", count);
