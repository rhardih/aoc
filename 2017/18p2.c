 ...
 * program 1 receives 0) and stores it in c. Each program now sees a
...
static long p0_registers[25];
static long p1_registers[25];
static char *program[INPUT_SIZE];

int main(int argc, char const *argv[])
{
  long *registers;
  long i, *pc, p0_pc, p1_pc, value, snd_count = 0;
  char instruction[4], reg_x, reg_y;
  char *line = NULL;
  size_t linecap = 0;
  regex_t type0, type1;

  // Use two-ringbuffers for messaging, assuming no more than 1000 unreceived
  // messages are in transit at any time.
  long msgbuffers[2][MSGBUFSIZE];
  int p0_midx = 0, p0_midxe = 0, p1_midx = 0, p1_midxe = 0;
  ...
  pc = &p0_pc;
  registers = p0_registers;

  while (p0_pc >= 0 && p0_pc < INPUT_SIZE && p1_pc >= 0 && p1_pc < INPUT_SIZE)
  {
    if (regexec(&type0, program[*pc], 0, NULL, 0) == 0)
    {
      sscanf(program[*pc], "%s %c %ld", instruction, &reg_x, &value);
      reg_x -= 'a';
    ...
    else if (regexec(&type1, program[*pc], 0, NULL, 0) == 0)
    {
      sscanf(program[*pc], "%s %c %c", instruction, &reg_x, &reg_y);
      reg_x -= 'a';
      reg_y -= 'a';
    ...
    {
      sscanf(program[*pc], "%s %c", instruction, &reg_x);
      reg_x -= 'a';
      ...
      if (strncmp(instruction, "snd", 3) == 0)
      {
        if (pc == &p0_pc)
        {
          msgbuffers[1][p1_midxe] = registers[reg_x];

          p1_midxe = (p1_midxe + 1) % MSGBUFSIZE;
        }
        else
        {
          msgbuffers[0][p0_midxe] = registers[reg_x];

          p0_midxe = (p0_midxe + 1) % MSGBUFSIZE;
          snd_count++;
        }
      }
      else if (strncmp(instruction, "rcv", 3) == 0)
      {
        if (pc == &p0_pc)
        {
          if (p0_midx == p0_midxe)
          {
            // both queues empty - deadlock
            if (p1_midx == p1_midxe)
              break;

            // no further messages, switch programs, without increasing the
            // program counter
            pc = &p1_pc;
            registers = p1_registers;
            continue;
          }
          else
          {
            registers[reg_x] = msgbuffers[0][p0_midx];

            p0_midx = (p0_midx + 1) % MSGBUFSIZE;
          }
        }
        else
        {
          ...
          {
            registers[reg_x] = msgbuffers[1][p1_midx];

            p1_midx = (p1_midx + 1) % MSGBUFSIZE;
          }
        }
      }
    }
  ...
  printf("Program 1 sent a value %ld times.\n", snd_count);
