...
{
  int depth, range, ps, caught = 1, delay;
  ...
  while (caught)
  {
    for (ps = 0, caught = 0; ps < MAX_DEPTH + 1; ++ps)
      if (scanners[ps] != 0 && (ps + delay) % ((scanners[ps] - 1) * 2) == 0)
        caught = 1;

    delay++;
  }

  printf("Fewest number of picoseconds needed to delay: %d\n", delay - 1);
