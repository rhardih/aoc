...
static int input = 355;

int main(int argc, char const *argv[])
{
  ll_t *spinlock = ll_list_create();
  ll_node_t *initial = ll_node_create(), *current, *new;
  uint64_t i, j, inserted = 1;

  spinlock->first = initial;
  ...
  printf("Value af after 2017: %llu\n", (uint64_t)spinlock->first->next->value);

  current = spinlock->first->next;

  while(current != spinlock->first)
  {
    new = current;
    current = current->next;
    free(new);
