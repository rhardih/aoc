...
int main(int argc, char const *argv[])
{
  ...
  int x = 12, xmax = 25, y = 12, ymax = 25, direction = UP;
  int bursts = 0, infections = 0, grid_size = xmax * ymax + 1;
  int i, j;

  char *grid = malloc(grid_size * sizeof(char)), *tmp;
  strcpy(grid, input);
    ...
    while(x > 0 && x < xmax && y > 0 && y < ymax)
    {
      ...
      for (i = 0; i < ymax; ++i)
      {
        for (j = 0; j < xmax; ++j)
        {
          if (j != 0)
            printf(" ");
          printf("%c", grid[i * xmax + j]);
        }
        printf("\n");
      }
      printf("\n");

      printf("Bursts: %d, infections: %d\n", bursts, infections);
#endif
    }

    // Expand grid to be nine times as big, plus spaces in between
    grid_size = (xmax * ymax) * 9 + 1;
    tmp = malloc(grid_size * sizeof(char));

    // Fill with clean nodes
    memset(tmp, '.', grid_size);
    grid[grid_size] = '\0';

    // Copy existing grid to center
    for (i = 0; i < ymax; i++)
      for (j = 0; j < xmax; j++)
        tmp[(i + ymax) * 3 * xmax + (j + xmax)] = grid[i * xmax + j];
