...
until p1_cards.empty? or p2_cards.empty?
  p1_pick = p1_cards.shift
  p2_pick = p2_cards.shift

  if p1_pick > p2_pick
    p1_cards.push p1_pick
    p1_cards.push p2_pick
  else
    p2_cards.push p2_pick
    p2_cards.push p1_pick
  end
end

winning_cards = (p1_cards.empty? ? p2_cards : p1_cards)

score = winning_cards.each_with_index.reduce(0) do |memo, (card, index)|
  memo += card * (winning_cards.length - index)
end

puts "The winning player's score: #{score}"
