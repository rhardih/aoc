...
top.split("\n").each do |line|
  ...
  case right
  when /a|b/
    rules[left] = right.scan(/[ab]/).first
  else
    rules[left] = right.split("|").map(&:strip).map { |rule| rule.split(" ") }
  end
end
...
expand = lambda do |k|
  ...
  return rule if rule.kind_of?(String)

  group = rule.map do |rule_list|
    rule_list.map { |r| expand.call(r) }.join
  end.join("|")

  return "(#{group})"
end

regex = Regexp.new("^#{expand.call("0")}$")

count = bottom.split("\n").reduce(0) do |c, line|
  c += 1 unless regex.match(line).nil?; c
end

puts "Messages completely matching rule 0: #{count}"
