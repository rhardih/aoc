...
int main(int argc, char **argv) {
  ...
  while(scanf("%d-%d %c: %s", &lowest, &highest, &letter, password) != EOF) {
    ...
    if (((password[lowest] == letter && password[highest] != letter) ||
        (password[lowest] != letter && password[highest] == letter)) &&
        password[lowest] != password[highest]) {
      count++;
    }
  }
