...
program = STDIN.read.split("\n")
memory = Hash.new
maskSplit = []
addressQueue = Queue.new
...
program.each do |line|
  ...
  when /^mask = ([01X]+)$/
    maskSplit = $1.split("")
  when /^mem\[(\d+)\] = (\d+)$/
    binSplit = $1.to_i.to_s(2).rjust(36, "0").split("")

    maskSplit.each_with_index do |d, i|
      binSplit[i] = maskSplit[i] if maskSplit[i] != "0"
    end

    addressQueue << binSplit.join

    until addressQueue.empty?
      address = addressQueue.pop

      if address["X"].nil?
        memory[address.to_i(2)] = $2.to_i
      else
        a0 = address.dup
        a0["X"] = "0"
        addressQueue << a0
...
puts "Sum of all values left in memory after completion: #{memory.values.sum}"
