...
void change_course(ship_t *s, instruction_t i) {
  int directions[] = { NORTH, EAST, SOUTH, WEST };
  int step = i.value / 90;
  int amount = i.action == 'L' ? 4 - step : step;

  s->d = directions[(s->d + amount) % 4];
}

void sail(ship_t *s, int distance) {
  switch(s->d) {
    case NORTH:
      s->y -= distance;
      break;
  ...
  while(scanf("%c%d\n", &i.action, &i.value) != EOF) {
    switch(i.action) {
      ...
      case 'F':
        ...
        break;
      default:
        printf("Action error: %c%d", i.action, i.value);
        break;
    }
  }
