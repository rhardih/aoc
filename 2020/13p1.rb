...
earliest_bus_index = nil
bus_ids = two.split(",").filter { |bi| bi != "x" }.map(&:to_i)
departures = bus_ids.dup

loop do
  earliest_bus_index = departures.find_index do |departure|
    departure >= earliest_bus_estimate
  end

  break if earliest_bus_index != nil

  departures.each_with_index do |d, i|
    departures[i] += bus_ids[i]
  end
end

wait = departures[earliest_bus_index] - earliest_bus_estimate

puts "ID of the earliest bus multiplied by the number of minutes you'll need " \
  "to wait for that bus: #{wait * bus_ids[earliest_bus_index]}"
