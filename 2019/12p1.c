...
int apply_gravity(moon_t *pairs[6][2]) {
  int i;
  moon_t *m0, *m1;
...
int apply_velocity(moon_t *moons[4]) {
  int i;
  moon_t *m;
...
int total_energy(moon_t *moons[4]) {
  int sum = 0, i;
...
int main() {
  ...
  scanf("<x=%d, y=%d, z=%d>\n", &io.x, &io.y, &io.z);
  scanf("<x=%d, y=%d, z=%d>\n", &europa.x, &europa.y, &europa.z);
  scanf("<x=%d, y=%d, z=%d>\n", &ganymede.x, &ganymede.y, &ganymede.z);
  scanf("<x=%d, y=%d, z=%d>\n", &callisto.x, &callisto.y, &callisto.z);
