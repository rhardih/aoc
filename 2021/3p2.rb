...
- In the third position, there are an equal number of 0 bits and 1 bits (one
  ...
  position: 01010.  As there is only one number left, stop; the CO2 scrubber
  rating is 01010, or 10 in decimal.
...
STDIN.read.split("\n").each { |line| rates << line.split("") }

def bit_counts(rates)
  ones = Array.new(rates.first.length)
  zeroes = Array.new(rates.first.length)

  (0...ones.length).each do |i|
    ones[i] = rates.map { |rate| rate[i] }.count("1")
    zeroes[i] = rates.map { |rate| rate[i] }.count("0")
  end
...
(0...tmp_rates.first.length).each do |index|
  ones, zeroes = bit_counts(tmp_rates)
  filter_digit = ones[index] >= zeroes[index] ? "1" : "0"

  tmp_rates = tmp_rates.select { |rate| rate[index] == filter_digit }

  break if tmp_rates.length == 1
end

oxygen_generator_rating = tmp_rates.first.join.to_i(2)
...
(0...tmp_rates.first.length).each do |index|
  ones, zeroes = bit_counts(tmp_rates)
  filter_digit = zeroes[index] <= ones[index] ? "0" : "1"
...
co2_scrubber_rating = tmp_rates.first.join.to_i(2)

puts "Life support rating of the submarine: #{co2_scrubber_rating * oxygen_generator_rating}"
