...
- [[3,[2,[1,[7,3]]]],[6,[5,[4,[3,2]]]]] becomes
  [[3,[2,[8,0]]],[9,[5,[4,[3,2]]]]] (the pair [3,2] is unaffected because the pair
  [7,3] is further to the left; [3,2] would explode on the next action).
...
sum = STDIN.read.split("\n").reduce([]) do |memo, line|
  memo.tap { |m| m << Pair.new(eval(line)) }
end.reduce do |node0, node1|
  node0 + node1
end

puts "Magnitude of the final sum: #{sum.magnitude}"
