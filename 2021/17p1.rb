...
x_range, y_range = STDIN.read.scan(
  /target area: x=([-+]?\d+..[-+]?\d+), y=([-+]?\d+..[-+]?\d+)/
).flatten.map { |range| eval(range) }

Probe = Struct.new(:x, :y, :vx, :vy)

search_limit = 100 # guesstimate
global_y_max = 0

0.upto(search_limit) do |xv|
  0.upto(search_limit) do |yv|
    probe = Probe.new(0, 0, xv, yv)
    local_y_max = 0
    ...
    loop do
      ...
      local_y_max = [probe.y, local_y_max].max

      probe.vx -= probe.vx <=> 0
      probe.vy -= 1

      within_target = x_range === probe.x && y_range === probe.y

      if within_target
        global_y_max = [local_y_max, global_y_max].max

        break
      end

      # Overshot target
      break if probe.y < y_range.min
      break if xv > 0 ? probe.x > x_range.max : probe.x < x_range.min
    end
  end
end

puts "Highest y position on trajectory: #{global_y_max}"
