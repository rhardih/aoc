...
binary_numbers = STDIN.read.split("\n").reduce([]) { |memo, line| memo << line }

gamma_rate = (0...binary_numbers.first.length).reduce("") do |memo, i|
  one_bits = binary_numbers.map { |bn| bn[i] }.count("1")
  zero_bits = binary_numbers.length - one_bits
  memo + (one_bits > zero_bits ? "1" : "0")
end

# Ones complement of the gamma_rate
epsilon_rate = gamma_rate.tr("01", "10")

puts "Power consumption of the submarine: #{gamma_rate.to_i(2) * epsilon_rate.to_i(2)}"
