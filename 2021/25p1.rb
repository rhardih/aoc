...
def move_east(map)
  ...
  # Move east facing
  (map.height - 1).downto(0).each do |y|
    (map.width - 1).downto(0).each do |x|
      next unless map[y][x] == ">"
      next unless map[y][(x + 1) % map.width] == "."
...
def move_south(map)
  ...
  (map.height - 1).downto(0).each do |y|
    (map.width - 1).downto(0).each do |x|
      next unless map[y][x] == "v"
      next unless map[(y + 1) % map.height][x] == "."

      tmp[y][x] = "."
      tmp[(y + 1) % map.height][x] = "v"
    end
  end
...
until map_before == map
  map_before = map.map(&:dup).dup

  map = move_east(map)
  map = move_south(map)

  puts "After #{i} step#{i > 1 ? "s" : ""}"
  puts map.join("\n")
  puts
