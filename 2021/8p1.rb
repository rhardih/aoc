...
count = STDIN.read.split("\n").map.reduce(0) do |acc0, line|
  outputs = line.split(" | ").map(&:split)[1]

  acc0 + outputs.reduce(0) do |acc1, output|
    match = output.length == 2 || output.length == 4 ||
...
puts "Number of times 1, 4, 7 or 8 appear: #{count}"
