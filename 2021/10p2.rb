...
- Multiply the total score by 5 to get 290, then add the value of > (4) to get
a new total score of 294.
...
completions = STDIN.read.split("\n").reduce([]) do |memo, line|
  stack = []
  corrupt = false

  line.chars.each do |c|
    if [')', ']', '}', '>'].include?(c)
      if closing[stack.last] != c
        # Syntax error
        # puts "Expected #{closing[stack.last]}, but found #{c} instead."
        corrupt = true
        break
      else
        stack.pop
  ...
  unless corrupt
    # puts "#{line} - Complete by adding #{completion}"
    memo << stack.reverse.map { |c| closing[c] }.join
  end
...
scores = completions.map do |completion|
  completion.chars.reduce(0) do |acc, c|
    acc *= 5
    acc += points[c]
    acc
  end
...
middle_score = scores.sort[scores.length / 2]

puts "Total syntax error score: #{middle_score}"
