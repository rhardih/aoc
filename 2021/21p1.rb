...
Player = Struct.new(:no, :position, :score)
...
loop do
  ...
  # puts "Player #{player.no} rolls #{rolls.join("+")} and moves to space #{player.position} for a total score of #{player.score}"

  player_no = player.no == 1 ? 2 : 1

  break if players.any? { |_, player| player.score >= 1000 }
end

loosing_player = players.min_by { |_, player| player.score }.last

puts "Score times number of dice rolls: #{loosing_player.score * dice_rolls}"
