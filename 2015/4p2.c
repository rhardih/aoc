...
int main(int argc, char const *argv[])
{
  ...
  while (num < UINT_MAX) {
    sprintf(candidate, "%s%u", input, num);
    ...
    if (strncmp(md5_string, "000000", 6) == 0)
      break;

    num++;
  }

  printf("The lowest possible number producing an md5 hash with six leading "
      "zeros is %u, giving the hash: %s\n", num, md5_string);
