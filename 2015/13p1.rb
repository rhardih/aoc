...
arrangements = people.permutation.map do |arrangement|
  happiness = arrangement.map.with_index  do |person, i|
    p_left = arrangement[i - 1]
    p_right = arrangement[(i + 1) % arrangement.length]
...
puts "Total change in happiness for the optimal seating arrangement: #{total_change}"
