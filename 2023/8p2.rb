...
i_network.split("\n").each do |line|
  node, left, right = line.scan(/([A-Z0-9]{3}) = \(([A-Z0-9]{3}), ([A-Z0-9]{3})\)/).first

  network[node] = [left, right]
end

current_nodes = network.select { |k| k.end_with?("A") }.keys

node_steps = current_nodes.map do |node|
  steps = 0
  current_node = node
...
steps = node_steps.reduce { |acc, s| acc.lcm(s) }

puts "Number of steps to nodes that end with Z: #{steps}"
