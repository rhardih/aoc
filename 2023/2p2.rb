...
result = STDIN.read.split("\n").map do |line|
  ...
  least_required = CubeSet.new(0, 0, 0)

  subsets.map do |subset|
    subset.split(", ").each do |picking|
      amount, color = picking.split

      if least_required[color] < amount.to_i
        least_required[color] = amount.to_i
      end
    end
  end

  least_required.red * least_required.green * least_required.blue
end.sum

puts "The sum of the power of cube sets: #{result}"
