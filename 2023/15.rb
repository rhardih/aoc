...
sum = sequence.reduce(0) { |acc, s| acc + hash(s) }

puts "Part 1: #{sum}"

Lens = Struct.new(:focal_length, :label)

boxes = Hash.new { |h,k| h[k] = [] }

input.strip.split(",").each do |step|
  label, operation, focal_length = step.scan(/(\w+)([=-])(\d)?/).first
  box = hash(label)
  lens = Lens.new(focal_length, label)

  if operation == "-"
    boxes[box].delete_if { |candidate_lens| candidate_lens.label == lens.label }
  else
    i = boxes[box].index { |candidate_lens| candidate_lens.label == lens.label }
  ...
  # puts "After \"#{step}\""
  # boxes.each do |k,v|
  #   content = v.map { |lens| "[#{lens.label} #{lens.focal_length}]" }
  #   puts "Box #{k}: #{content.join(" ")}" unless v.empty?
  # end
  # puts
end

sum = (0..256).reduce(0) do |acc, bn|
  box = boxes[bn]

  acc + box.each.with_index.reduce(0) do |i_acc, (lens, i)|
    fp = (bn + 1) * (i + 1) * lens.focal_length.to_i
    # puts "#{lens.label}: #{lens.focal_length} (box #{bn}) * #{i + 1} * #{lens.focal_length} = #{fp}"
