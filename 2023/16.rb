...
def energized_tiles(input, beam)
  beams = [beam]

  energy_map = Array.new(input.length) { Array.new(input.first.length, [".", nil]) }

  until beams.empty? do
    row, col, direction = beams.pop

    if row < 0 || row == input.length ||
        col < 0 || col == input.first.length
      next
    end

    # A ray with the same direction already passed through here
    next if energy_map[row][col] == ["#", direction]

    energy_map[row][col] = ["#", direction]
...
result = energized_tiles(input, [0, 0, :r])
...
(0..input.length).each do |row|
  results << energized_tiles(input, [row, 0, :r])
  results << energized_tiles(input, [row, input.first.length - 1, :l])

  if row == 0
    (0..input.first.length).each do |col|
      results << energized_tiles(input, [row, col, :d])
      results << energized_tiles(input, [row, col, :r]) if col == 0
      results << energized_tiles(input, [row, col, :l]) if col == input.first.length - 1
    end
  end

  if row == input.length - 1
    (0..input.first.length).each do |col|
      results << energized_tiles(input, [row, col, :u])
      results << energized_tiles(input, [row, col, :r]) if col == 0
      results << energized_tiles(input, [row, col, :l]) if col == input.first.length - 1
    end
  end
