...
def lit?(x, cycle)
  # Sprite position rounded within nearest vertical line
  index = x + (cycle / 40) * 40
  index - 1 == cycle || index == cycle || index + 1 == cycle
end

STDIN.read.split("\n").each do |line|
  case line
  when /addx (-?\d+)/
    crt[cycle] = lit?(x, cycle) ? "#" : "."
    cycle += 1
...
puts "Eight capital letters appearing on your CRT:"
puts crt.scan(/.{40}/).join("\n")
