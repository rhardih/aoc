...
{
  int winner;
  int prev_last_elf_number;
  int last_elf_number = 1;

  while (last_elf_number < INPUT)
  {
    prev_last_elf_number = last_elf_number;
    last_elf_number = last_elf_number * 3;
  }

  prev_last_elf_number = last_elf_number / 3;

  // For this second part, the principle remains the same but the series
  // changes. The last elf now gets all the gifts, whenever the number of elfs
  // belongs to this series:
  //
  //   n = (n - 1) * 3, n > 0
  //
  // Another difference in this case is, that the solution is not always the
  // difference to the next last elf number greater than the number of elves; N.
  // When the number of elves is less than or equal to the last elf number
  // previous to N; M, the solution is then the difference between the number of
  // elves and M.

  if (INPUT - prev_last_elf_number <= prev_last_elf_number)
    winner = INPUT - prev_last_elf_number;
  else
    winner = INPUT - (last_elf_number - INPUT);
