 ...
 * trap.  Only its right tile is a trap.  In any other situation, the new tile
 * is safe.
 * 
...
{
  int st = 0, i, j, r1, r2, r3, r4, lts, cts, rts;
  char room[2][INLEN + 1] = { 0 };

  fgets(room[0], INLEN + 1, stdin);
  ...
  for (i = 1; i < ROWS; i++)
  {
    for (j = 0; j < INLEN; j++)
    {
      lts = j == 0 || room[(i - 1) % 2][j - 1] == '.';
      cts = room[(i - 1) % 2][j] == '.';
      rts = j == INLEN - 1 || room[(i - 1) % 2][j + 1] == '.';
