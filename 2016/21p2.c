...
// 1 2 3 4 6 7 8 9  <=>  9 1 6 2 7 3 8 4 
// 1 3 5 7 2 4 6 0 
void rotate_position(char s[LINPUT], char x) {
  char *a = strchr(s, x);
  int index = a - s;
  static int lookup[] = { 9, 1, 6, 2, 7, 3, 8, 4 };

  rotate_left(s, lookup[index] % (LINPUT - 1));
}
...
{
  int i = 0, x, y;
  char buf[100][50], rbuf[100][50], a, b;
  char scrambled[LINPUT] = INPUT;

  while(fgets(buf[i++], 50, stdin) != NULL) { }

  for (i--; i >= 0; i--)
  {
    if (sscanf(buf[i], "swap position %d with position %d", &x, &y)) {
      swap_position(scrambled, x, y);
    } else if (sscanf(buf[i], "swap letter %c with letter %c", &a, &b)) {
      swap_letter(scrambled, a, b);
    } else if (sscanf(buf[i], "rotate left %d steps", &x)) {
      rotate_right(scrambled, x);
    } else if (sscanf(buf[i], "rotate right %d steps", &x)) {
      rotate_left(scrambled, x);
    } else if (sscanf(buf[i], "rotate based on position of letter %c", &a)) {
      rotate_position(scrambled, a); //
    } else if (sscanf(buf[i], "reverse positions %d through %d", &x, &y)) {
      reverse_positions(scrambled, x, y);
    } else if (sscanf(buf[i], "move position %d to position %d", &x, &y)) {
      move_position(scrambled, x, y);
    }
  }

  printf("Result of un-scrambling input %s: %s\n", INPUT, scrambled);
